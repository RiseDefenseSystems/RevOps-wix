...
export function risedefensesystems_Router(request) {
   ...
   if (data) {
       ...
       // Render item page 
       return ok("risedefensesystems-page", data, seoData); 
   }
...
export function risedefensesystems_SiteMap(sitemapRequest) {
   ...
   const siteMapEntries = Object.keys(peopleData).map((name)=>{
                               ...
                               entry.pageName = "risedefensesystems-page";		// The name of the page in the Wix editor to render
                               entry.url = "/risedefensesystems/" + name ;			// Relative URL of the page
